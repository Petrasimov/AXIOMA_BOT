...
# Разделы-заглушки. Пусто: все пункты меню реализованы.
# Словарь оставлен как точка расширения — если появится новый раздел,
# его можно временно показать заглушкой, не трогая роутер.
...
async def handle_menu_callback(callback: dict, action: str):
    ...
    Сюда попадают только те действия, которые роутер не разобрал сам:
    возврат в меню и заглушки нереализованных разделов.
    """
